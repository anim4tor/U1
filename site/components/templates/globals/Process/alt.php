				...
				<div class="grid inner-x__1" data-scroll >
					<?php foreach (collection('Process') as $step) : ?>
						<div data-tab="step-<?= $step->step()?>" class="flex align__start gap__02" >
							<?= snippet('atoms/Heading', [ 'level' => 'h3', 'text' => $step->label(), 'reveal' => true, 'css' => 'l', 'node' => 'data-split-ignore' ]) ?>
							<div data-reveal-text="" class="wrap-t__03 m" data-split-ignore style="--in-delay: 800ms">(<?= $step->step() ?>)</div>
						</div>
					<?php endforeach ?>
				</div>
