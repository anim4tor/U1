<li class="item" data-scroll>
	<a href="<?= $feed->url() ?>" class="grid wrap">	
		<?php if ($cover = $feed->cover()->toFile()) : ?>
			<div class="item__figure grid img__radius no__overflow "><?= snippet('atoms/Image', ['img' => $cover, 'parallax' => 1, 'reveal' => true, 'css' => 'h__12 grid' ]) ?></div>
		<?php endif ?>
		<div class="flex justify__space-between align__start gap__5 inner-y__05 ">
			<div class="flex gap__2 upper ">
				...
				<h4 class="font__size__5 s" data-reveal-text="lines"><?= $feed->title() ?></h4>
			</div>
