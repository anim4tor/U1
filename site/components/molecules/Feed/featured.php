<li class="" data-scroll>
	<div class="grid__2 gap__2 inner-y__1 border__top wrap">	
		<div class="grid place__space-between-stretch gap__2 wrap-r__5 inner-r__2 ">
			<div class="grid">
				<p class="upper ">(Featured)</p>
				<span class="op__4"><?= $feed->date()->toDate('Y-m-d') ?></span>
			</div>
			<a href="<?= $feed->url() ?>" ><h3 class=""><?= $feed->title() ?></h3></a>
		...
		<a href="<?= $feed->url() ?>" class="grid">
			<div class="item__figure grid img__radius no__overflow -wrap-l__5"><?= snippet('atoms/Image', ['img' => $cover, 'parallax' => 1, 'css' => 'vh__20 grid ' ]) ?></div>
		</a>
		<?php endif ?>
		<!-- <div class="grid ">
