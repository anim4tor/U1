...
<?php if ($url) : ?>
	<a href="<?= $url ?>" <?= $target ? 'target="_blank" rel="noopener"' : null ?> class="button uppercase <?= $css ?>" <?= $theme ? 'theme="'.$theme.'"' : null ?> <?= $hover ? 'hover="'.$hover.'"' : null ?> <?= $reveal ? 'data-reveal-text' : '' ?> <?= $node ?>>
		<?php if ($label) : ?>
...
<?php else: ?>
	<button class="button uppercase <?= $css ?>" <?= $node ?> <?= $theme ? 'theme="'.$theme.'"' : null ?> <?= $hover ? 'hover="'.$hover.'"' : null ?> <?= $reveal ? 'data-reveal-text' : '' ?>>
		<?php if ($label) : ?>
			<span aria-label="<?= $label ?><?= $counter !== false ? ' (' . $counter . ')' : '' ?>">
				<?= $label ?><?= $counter !== false ? ' (' . $counter . ')' : '' ?>
			</span>
